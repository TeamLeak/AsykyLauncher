...
import {
  Box, Accordion, AccordionItem, AccordionButton, AccordionPanel, AccordionIcon, Flex, Text, Slider, SliderTrack, SliderFilledTrack, SliderThumb, Input, Select, Table, Thead, Tbody, Tr, Th, Td, Switch, useColorModeValue, Tooltip, Skeleton
} from '@chakra-ui/react';
import { useTranslation } from 'react-i18next';

const GameSettings: React.FC<{ tempSettings: any | null; handleInputChange: (key: string, value: any) => void; }> = ({ tempSettings, handleInputChange }) => {
  ...
  const handleModToggle = (id: number) => {
    setMods(prevMods =>
      prevMods.map(mod =>
        ...
        <AccordionPanel pb={4} px={4}>
          ...
          <Table variant="simple">
            ...
            <Tbody>
              {mods.map((mod) => (
                <Tr key={mod.id}>
                  <Td><Switch isChecked={mod.enabled} onChange={() => handleModToggle(mod.id)} /></Td>
                  <Td>{mod.name}</Td>
                  <Td>{mod.version}</Td>
                  <Td>{mod.dateAdded}</Td>
                </Tr>
