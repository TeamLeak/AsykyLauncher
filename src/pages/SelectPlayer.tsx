...
import { Box, Flex, Text, Image, IconButton, Tooltip, Menu, MenuButton, MenuList, MenuItem, Portal, Spinner, Modal, ModalOverlay, ModalContent, ModalHeader, ModalBody, ModalCloseButton, useDisclosure } from '@chakra-ui/react';
...
const SelectPlayer: React.FC = () => {
  ...
  const [contextMenu, setContextMenu] = useState<{ playerId: number | null; playerElement: HTMLElement | null }>({ playerId: null, playerElement: null });
  ...
  const handlePlayerClick = async () => {
    ...
    // Increment attempt count before the request
    ...
    if (response === 'success') {
      ...
      setTimeout(() => {
        onClose();
        setTimeout(() => navigate('/'), 500); // Delay before navigating
      }, 500);
    } else {
      setStatus('error');
      setTimeout(() => {
        onClose();
        setTimeout(() => navigate('/login'), 500); // Delay before navigating
      }, 500);
    }
  };
  ...
  const renderPlayers = (startIndex: number, endIndex: number) => (
    <Flex justify="center" mt={10}>
      {players.slice(startIndex, endIndex).map(player => (
        <Box key={player.id} mx={4} textAlign="center" position="relative" role="group" onContextMenu={(event) => handleContextMenu(event, player.id)}>
          <Image
            src={player.avatar}
            alt={player.name}
            boxSize="100px"
          ...
          <Portal>
            ...
            >
              <Menu isOpen onClose={handleClose}>
                <MenuButton />
                <MenuList>
                  <MenuItem onClick={() => alert(`Open player ${contextMenu.playerId}`)}>{t('open')}</MenuItem>
                  <MenuItem color="red" onClick={() => alert(`Logout player ${contextMenu.playerId}`)}>{t('logout')}</MenuItem>
                </MenuList>
              </Menu>
            </Box>
          </Portal>
